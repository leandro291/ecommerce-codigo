...
8. Rutas y endpoints de admin protegidos en `proxy.ts` (Next 16, ex-`middleware.ts`) **y** con verificación por código de permiso en el handler (`requirePermission('products.create')`). Comparar nombres de rol en el código (`role === 'admin'`) es hallazgo bloqueante.
